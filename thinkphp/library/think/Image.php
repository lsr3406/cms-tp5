...
class Image
{
    ...
    /**
     * 将原图像按照保留最大图片内容信息的原则等比例压缩成固定宽和高的图片(有裁剪)
     * @param  integer $width  缩略图的宽度
     * @param  integer $height 缩略图的高度
     * @return $this
     */
    public function thumbTo($dst_w, $dst_h)  {
        ...
        if ($dst_ratio > $src_ratio){
            ...
            $h = $w / $dst_ratio;
            $x = 0;
            $y = ($src_h - $dst_h*$src_w/$dst_w)/2;
        } else {
            $h = $src_h;
            $w = $h * $dst_ratio;
            $y = 0;
            $x = ($src_w - $dst_w*$src_h/$dst_h)/2;
        }
        $img = imagecreatetruecolor($dst_w, $dst_h);
        // 调整默认颜色
        // $color = imagecolorallocate($img, 255, 255, 255);
        // imagefill($img, 0, 0, $color);
        imagecopyresampled($img, $this->im, 0, 0, $x, $y, $dst_w, $dst_h, $w, $h);
        imagedestroy($this->im); //销毁原图
        $this->im = $img;
        $this->info['width']  = (int) $dst_w;
        $this->info['height'] = (int) $dst_h;
        return $this;
    }
